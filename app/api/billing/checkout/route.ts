...
export async function POST(request: Request) {
    try {
        ...
        // subscription has ever existed: signup pre-fills trial_ends_at on
        // every new row, so that column can't distinguish a brand-new account
        // from one that's already had its fortnight. The id is kept on the row
        // after a cancellation, so anyone who cancels and comes back is billed
        // immediately instead of trialling twice.
